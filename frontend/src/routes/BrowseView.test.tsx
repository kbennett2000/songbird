...
describe("BrowseView", () => {
  ...
  it("points at the Sermon sources page from the sermon controls", async () => {
    // Spec §10: the page is reachable from the top nav AND from here, next to export/import —
    // this is where someone is already looking at their sermon notes.
    server.use(browseHandler(), sermonHandler());
    renderBrowse();

    const link = await screen.findByRole("link", { name: "Sermon sources" });
    expect(link).toHaveAttribute("href", "/sermon-sources");
  });
});
