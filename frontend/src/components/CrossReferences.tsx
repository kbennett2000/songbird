...
export function CrossReferences({
  ...
  if (query.isPending) return <p className="text-sm text-gray-500 dark:text-gray-400">Loading cross-references…</p>;
  if (query.isError)
    return <p className="text-sm text-red-600 dark:text-red-400">Couldn&rsquo;t load (is Concord reachable?).</p>;
  if (query.data.length === 0)
    return <p className="text-sm text-gray-500 dark:text-gray-400">No cross-references for this verse.</p>;
        ...
        <li key={`${ref.book}-${ref.chapter}-${ref.verse_start}`}>
          ...
          >
            <span className="flex items-baseline gap-2">
              <span className="font-medium text-blue-700 dark:text-blue-400">{ref.reference}</span>
              {ref.votes !== null && (
                <span className="text-xs text-gray-400 dark:text-gray-500">{ref.votes} votes</span>
              )}
            </span>
            {ref.text && <span className="mt-0.5 block text-sm text-gray-600 dark:text-gray-300">{ref.text}</span>}
          </button>
        </li>
      ))}
    </ul>
